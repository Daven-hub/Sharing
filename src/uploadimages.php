...
$dest_dossier = 'Book_Images/'; //nom du dossier ou vous allez stocké vos images
$dest_fichier="";

//**************************************
if(!$getimagesize = getimagesize($_FILES['image']['tmp_name'])) {
$erreurs[] = "Le fichier n'est pas une image valide.";
}
//*******************************
else {
if( (!in_array( get_extension($_FILES['image']['name']), $extensions_ok ))or (!in_array($getimagesize[2], $typeimages_ok )))
{
...
if( file_exists($_FILES['image']['tmp_name']) and filesize($_FILES['image']['tmp_name']) > $taille_max)
{
...
//}
//********************************
if(move_uploaded_file($_FILES['image']['tmp_name'], $dossier . $dest_fichier)) {
